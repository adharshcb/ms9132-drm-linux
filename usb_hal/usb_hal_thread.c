...
static int usb_hal_dev_draw_cursor(struct usb_hal_dev* usb_dev)
{
	...
	int cursor_start_row, cursor_start_col;
	int cursor_rows, cursor_cols;
	int img_stride;
	struct usb_hal_cursor_buffer* cursor_buf;

	/* Validate usb_dev and buffers */
	if (!usb_dev || !usb_dev->desktop_buf.buf)
		return -EINVAL;

	img_stride = usb_dev->mode.width * 4;

	/* Validate mode dimensions */
	if (usb_dev->mode.width <= 0 || usb_dev->mode.height <= 0)
		return -EINVAL;

	//remove old cursor if exist
	cursor_buf = &usb_dev->old_cursor_buf;
	if (cursor_buf->valid && cursor_buf->buf) {
		...
		/* Bounds check for old cursor restore */
		if (start_row >= 0 && start_col >= 0 &&
		    end_row <= usb_dev->mode.height && end_col <= usb_dev->mode.width &&
	...
	if (cursor_buf->valid && cursor_buf->buf && usb_dev->old_cursor_buf.buf) {
		...
		if (start_row >= end_row || start_col >= end_col) {
			...
			/* Calculate cursor buffer offset and validate bounds */
			cursor_start_row = start_row - cursor_buf->y;
			cursor_start_col = start_col - cursor_buf->x;
			cursor_rows = end_row - start_row;
			cursor_cols = end_col - start_col;

			/* Ensure cursor buffer access is within bounds */
			if (cursor_start_row < 0 || cursor_start_col < 0 ||
			    cursor_start_row + cursor_rows > USB_HAL_CURSOR_HEIGHT ||
			    cursor_start_col + cursor_cols > USB_HAL_CURSOR_WIDTH) {
				usb_dev->old_cursor_buf.valid = false;
				return -EINVAL;
			}
			...
			//copy back the old mouse
			pimage = usb_dev->desktop_buf.buf + start_row * img_stride + start_col * 4;
			pmouse = usb_dev->cursor_buf.buf + (cursor_start_row * USB_HAL_CURSOR_WIDTH
						 + cursor_start_col) * 4;
...
static void usb_hal_image_to_yuv(struct usb_hal_dev* usb_dev)
{
	...
	/* Validate device and buffers */
	if (!usb_dev || !usb_dev->usb_buf.buf || !usb_dev->desktop_buf.buf)
		return;

	/* Validate frame index */
	if (usb_dev->frame_index < 0 || usb_dev->frame_index > 1)
		return;

	block_width = usb_dev->rect[usb_dev->frame_index].right - usb_dev->rect[usb_dev->frame_index].left;
	block_height = usb_dev->rect[usb_dev->frame_index].bottom - usb_dev->rect[usb_dev->frame_index].top;

	/* Validate block dimensions */
	if (block_width <= 0 || block_height <= 0)
		return;

	pdst = usb_dev->usb_buf.buf + 8;
	pline = usb_dev->desktop_buf.buf + (usb_dev->rect[usb_dev->frame_index].top * usb_dev->mode.width * 4)
...
static void usb_hal_update_change_rects(struct usb_hal_dev* usb_dev)
{
	...
	int width, height, line_stride;
	u8 *p_old, *p_new;

	/* Validate device and buffers before accessing */
	if (!usb_dev || !usb_dev->image_buf.buf || !usb_dev->desktop_buf.buf)
		return;
	...
	line_stride = width * 4;
	p_old = usb_dev->image_buf.buf;
	p_new = usb_dev->desktop_buf.buf;
